...
router.post('/', async (req, res) => {
  try {
    const { name, email, phone, preferredDate, preferredTime, message, customField } = req.body;
    ...
    const testDrive = new TestDrive({
      ...
      message,
      customField: {
        carexchange: customField?.carexchange || '',
        make: customField?.make || '',
        model: customField?.model || '',
        year: customField?.year || '',
        message: customField?.message || '',
        carId: customField?.carId || ''
      }
    });
