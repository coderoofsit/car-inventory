...
const AdminPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/cars/filter?limit=10`)
      ...
      .then(data => {
        if (data.success && Array.isArray(data.cars)) {
          ...
          const duplicatedCars = [...data.cars, ...data.cars.map(c => ({ ...c, _id: c._id + '_copy' }))];
          setCars(duplicatedCars);
        } else {
          setCars([]); // Set to empty array on failure
        }
      })
      .catch(() => {
    ...
    getGhlLocations().then(data => {
      if (data.locations) {
        // Duplicate data for demonstration
        const duplicatedLocations = [...data.locations, ...data.locations.map(l => ({ ...l, id: l.id + '_copy' }))];
        setLocations(duplicatedLocations);
      } else {
        setLocations([]);
      }
    }).catch(() => setLocations([]));
